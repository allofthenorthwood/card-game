...
const ResizeText = ({ children, maxFontSize }: { children: string, maxFontSize: number }) => {
  const containerRef = useRef() as React.MutableRefObject<HTMLDivElement>;
  const innerRef = useRef() as React.MutableRefObject<HTMLDivElement>;

  const [fontSize, setFontSize] = useState(maxFontSize || 20);
  useLayoutEffect(() => {
  ...
  return <Container ref={containerRef} fontSize={fontSize} height={maxFontSize}>
    <span ref={innerRef}>
    {children}
    </span>
  </Container>;
